...
input.on('message', (deltaTime, message) => {
    // CC's
    if(message[0] == 176){ 
        if(Object.values(ROTARY).indexOf(message[1]) !== -1) rotary(message, false, deltaTime);
        if(Object.values(FADER).indexOf(message[1]) !== -1) fader(message, false, deltaTime);
    }

    // Notes
    if(message[0] == 144) {
        if(Object.values(ROTARY_BUTTON).indexOf(message[1]) !== -1) rotary(message, true, deltaTime);
        if(Object.values(FADER_TOUCH).indexOf(message[1]) !== -1) fader(message, true, deltaTime);
        if(message[1] >= 8 && message[1] <= 39) button(message, deltaTime);
    }
});

function rotary(message, pressed, delta) {
    if(pressed && message[2] === 127) midiEvent.emit('rotary_press', getKeyByValue(ROTARY_BUTTON, message[1]), 'on');
    if(pressed && message[2] === 0) midiEvent.emit('rotary_press', getKeyByValue(ROTARY_BUTTON, message[1]), 'off');
    if(!pressed) midiEvent.emit('rotary_turn', getKeyByValue(ROTARY, message[1]), message[2], delta);
}

function fader(message, touch, delta) {
    if(touch && message[2] === 127) midiEvent.emit('fader_touch', getKeyByValue(FADER_TOUCH, message[1]), 'on');
    if(touch && message[2] === 0) midiEvent.emit('fader_touch', getKeyByValue(FADER_TOUCH, message[1]), 'off');
    if(!touch) midiEvent.emit('fader_move', getKeyByValue(FADER, message[1]), message[2], delta);
}
...
function button(message, delta) {
    ...
    if(message[1] >= 24 && message[1] <= 31) {
        buttonName = 'mute';
        strip = message[1] - 23;
    }
    if(message[1] >= 32 && message[1] <= 39) {
        buttonName = 'select';
        strip = message[1] - 31;
    }
